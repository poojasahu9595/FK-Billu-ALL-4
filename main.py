...
class ProductionApp:
    ...
    def __init__(self):
        ...
        # Multi-session scraper (will initialize later)
        ...
        ) if FEATURES['enable_duplicate_filter'] else None
        
        self.telegram = Telegram(
            workers=SCRAPER_CONFIG.get('telegram_workers', 25),
            debug=False
        ) if FEATURES['enable_telegram_notifications'] else None
    ...
    async def initialize(self):
        ...
        if success:
            ...
            print("\n⚠️ Some sessions failed, but will continue with available ones")
    ...
    async def scrape_url_tracked(self, url: str, idx: int, total: int) -> tuple:
        ...
        if products:
            ...
            print(f"[{idx}/{total}] {category[:40]:<40} | {len(products):>4} products | {elapsed:>5.1f}s | {speed:>5.1f}/s")
        ...
        for i in range(0, len(urls), batch_size):
            ...
            for r in results:
                if isinstance(r, tuple):
                    count, products, elapsed, speed = r
                    all_found += count
            ...
            # Save batch to storage (async, non-blocking)
            ...
            print(f"{'-'*90}")
            print(f"Batch {batch_num}/{total_batches}: {len(batch_products)} products in {batch_elapsed:.1f}s | Avg: {avg_speed:.1f} prod/s")
            
            # Calculate ETA
            if batch_num < total_batches:
                remaining = len(urls) - (i + len(batch))
                avg_time_per_url = total_elapsed / (i + len(batch))
    ...
    async def cycle(self, num: int):
        ...
        print(f"  Speed: {found/cycle_elapsed:.1f} products/second")
        print(f"{'='*80}\n")
